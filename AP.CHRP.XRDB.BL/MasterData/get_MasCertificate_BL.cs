...
{
    class get_MasCertificate_BL : AC_BC_BusinessLogic
    {

        public get_MasCertificate_BL(ref AC_MSSQL_Access dbc)
        {
            m_dbc = dbc;
        }
        ...
        public int get_MasCertificate(ref get_MasCertificate_IP ip, ref get_MasCertificate_OP op)
        {
            ...
            List<TBL_MAS_CERTIFICATE> ml_Certificate = new List<TBL_MAS_CERTIFICATE>();

            m_rc = m_dbc.Select<TBL_MAS_CERTIFICATE>(ref m_ErrorMessage, m_WhereCondition, ref ml_Certificate);
            if (m_rc < 0)
            {
